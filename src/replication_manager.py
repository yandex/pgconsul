        ...
        try:
            ...
            for replica in info:
                if replica['reply_time_ms'] / 1000 < self._zk_fail_timestamp:
                    should_wait = True
                    logging.debug('Replica %s have reply_time less than _zk_fail_timestamp %d', replica['client_hostname'], self._zk_fail_timestamp)
            if should_wait:
...
def _get_needed_replication_type(config, db, db_state, ha_replics):
    ...
    if 'count' in metric:
        if replics_number == 0:
            logging.debug("Needed repl type is async, because there is no alive replicas")
            return 'async'
    ...
    if 'load' in metric:
        ...
        try:
            ...
            ratio = 0.0
        if ratio >= over:
            logging.debug("Needed repl type is async, because current load ratio %f > overload_sessions_ratio %f",
                          ratio, over)
            return 'async'
