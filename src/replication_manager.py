...
class ReplicationManagerConfig:
    ...
    quorum_halves_above_majority: int
...
class QuorumReplicationManager(ReplicationManager):
    ...
    def _quorum_size(self, replicas_number):
        """
        Number of replicas that have to confirm a commit: a majority of the cluster raised
        by `quorum_halves_above_majority` halves of a replica, rounded up. Two halves always
        buy one more replica, one buys it on clusters with an odd number of hosts.
        """
        quorum_size = (replicas_number + 1 + self._config.quorum_halves_above_majority) // 2
    ...
    def _primary_quorum_size(self, sync_quorum):
        ...
        hosts disagree about `quorum_halves_above_majority` while it is rolled out, and a size
        larger than the primary's would let us promote a host that misses confirmed
        commits. An unusable value falls back to the majority, the smallest size the
        setting allows and the threshold this check used before the setting existed.
        """
        ...
        logging.warning(
            ...
            'falling back to the majority of the cluster.',
            last_primary,
            sync_quorum,
            primary_ssn,
        )
