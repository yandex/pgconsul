# ADR-0002: Стратегия распространения исключений до границы итерации
...
Исключения распространяются через `run_iteration()` до обработчика в `start()`,
который логирует ошибку и запускает следующую итерацию.
...
Нужно различать распространение исключений до обработчика в `start()` и обработку в точке
...
Принять модель распространения исключений **«python-like»** с тремя уровнями:
...
| Загрузка WAL после успешного promote | `pg._upload_wals()` | Необязательный завершающий шаг: его ошибка не должна превращать успешный promote в ошибку. Метод намеренно перехватывает `Exception`. |

Правила обработки:

1. Можно перехватывать **только** `PostgresConnectionError`: остальные исключения должны распространяться. Исключение — `pg._upload_wals()` после успешного promote, который намеренно перехватывает `Exception`.
...
4. Операция **не должна** вызываться из критической секции switchover/failover. Исключение —
   `pg._upload_wals()` как best-effort шаг уже успешного promote.
...
### Отрицательные
- ❌ **Риск при переходе:** перед удалением `@return_none_on_error` нужно проверить вызывающий код: пропущенный локальный обработчик может привести к нежелательному перезапуску итерации через `start()`.
