...
QuorumReplicationManager = _rm.QuorumReplicationManager
SingleSyncReplicationManager = _rm.SingleSyncReplicationManager
...
def _make_quorum_manager():
    db = MagicMock()
    zk = MagicMock()
    ssn_manager = MagicMock(spec=SsnManager)
    manager = QuorumReplicationManager(_make_config(), db, zk, ssn_manager)
    return manager, db, zk, ssn_manager


def _make_single_sync_manager():
    db = MagicMock()
    zk = MagicMock()
    ssn_manager = MagicMock(spec=SsnManager)
    manager = SingleSyncReplicationManager(_make_config(), db, zk, ssn_manager)
    return manager, db, zk, ssn_manager


# ---------------------------------------------------------------------------
# QuorumReplicationManager.set_ssn_before_promote
# ---------------------------------------------------------------------------

class TestQuorumSetSsnBeforePromote:

    def test_success_delegates_to_ssn_manager(self):
        """Switchover: side_replicas + extra_host are assembled and SSN is applied."""
        manager, db, zk, ssn = _make_quorum_manager()
...
# ---------------------------------------------------------------------------
# SingleSyncReplicationManager.set_ssn_before_promote
# ---------------------------------------------------------------------------

class TestSingleSyncSetSsnBeforePromote:

    def test_returns_true_without_any_calls(self):
        manager, db, zk, ssn = _make_single_sync_manager()
        ...
        ssn.apply_and_persist.assert_not_called()
