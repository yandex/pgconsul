...
def test_promote_falls_back_to_the_default_quorum_if_the_primary_ssn_is_unusable():
    ...
    for unusable in (None, '', 'None', 'pgconsul1_test', ssn(0, HOSTS[:4]), ssn(5, HOSTS[:4])):
        ...
        assert promote_safe(HOSTS[:4], HOSTS[:2], unusable, quorum_includes_primary=False) is False
        assert promote_safe(HOSTS[:4], HOSTS[:3], unusable, quorum_includes_primary=False) is True
