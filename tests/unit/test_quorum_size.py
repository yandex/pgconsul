...
def make(quorum_halves_above_majority: int, db=None, zk=None) -> QuorumReplicationManager:
    config = ReplicationManagerConfig(
        ...
        quorum_halves_above_majority=quorum_halves_above_majority,
    )
...
def installed_ssn(halves: int, replicas: int) -> str:
    db = MagicMock()
    make(halves, db=db).change_replication_to_quorum(HOSTS[:replicas])
    return db.change_replication_type.call_args[0][0]


def installed_quorum(halves: int, replicas: int) -> int:
    return int(installed_ssn(halves, replicas).split(' ')[1].split('(')[0])


def applied_ssn(halves: int, quorum_hosts, current_ssn, recorded_quorum) -> list:
    """
    ...
    make(halves, db=db, zk=zk).update_replication_type(db_state, quorum_hosts)
...
def promote_safe(
    ...
    return make(quorum_halves_above_majority, zk=zk).is_promote_safe(alive_hosts, replica_infos)
...
def test_every_two_halves_buy_one_more_replica():
...
def test_an_installed_quorum_is_left_alone():
    assert applied_ssn(1, HOSTS[:2], ssn(2, HOSTS[:2]), HOSTS[:2]) == []
...
def test_promote_follows_the_primary_not_the_local_setting():
    ...
    assert promote_safe(HOSTS[:2], HOSTS[:1], ssn(1, HOSTS[:2]), quorum_halves_above_majority=1) is False


def test_promote_falls_back_to_the_majority_if_the_primary_ssn_is_unusable():
    ...
    for unusable in (None, '', 'None', 'pgconsul1_test', ssn(0, HOSTS[:4]), ssn(5, HOSTS[:4])):
        ...
        assert promote_safe(HOSTS[:4], HOSTS[:2], unusable, quorum_halves_above_majority=1) is False
        assert promote_safe(HOSTS[:4], HOSTS[:3], unusable, quorum_halves_above_majority=1) is True
    assert promote_safe(HOSTS[:4], HOSTS[:2], ssn(2, HOSTS[:4]), primary=None) is False
...
def test_promote_falls_back_when_the_primary_ssn_cannot_be_read():
    ...
    for failing in (FakeZk.LAST_PRIMARY_PATH, FakeZk.get_ssn_value_path(PRIMARY)):
        assert promote_safe(HOSTS[:2], HOSTS[:1], ssn(2, HOSTS[:2]), raises=[failing]) is False
