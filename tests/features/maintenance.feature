Feature: Check maintenance mode
    ...
    @maintenance_exit
    Scenario: Single-host cluster should exit from the maintenance mode when Postgres is dead.
    ...
    Scenario: Check container stop in maintenance mode
        Given a "pgconsul" container common config
        """
            pgconsul.conf:
                global:
    ...
    Scenario: Check pgbouncer is untouchable in maintenance mode
        Given a "pgconsul" container common config
        """
            pgconsul.conf:
                global:
    ...
    Scenario: Sync replication turns off in maintenance
        Given a "pgconsul" container common config
        """
            pgconsul.conf:
                global:
        ...
        Then container "postgresql2" is in quorum group
	@maintenance_primary
    Scenario: Node with current primary exists in maintenance
        Given a "pgconsul" container common config
        """
            pgconsul.conf:
                global:
        ...
        When we set value "enable" for key "/pgconsul/postgresql/maintenance" in zookeeper "zookeeper1"
        And we wait "10.0" seconds
        Then zookeeper "zookeeper1" has value "pgconsul_postgresql1_1.pgconsul_pgconsul_net" for key "/pgconsul/postgresql/maintenance/master"
        When we set value "disable" for key "/pgconsul/postgresql/maintenance" in zookeeper "zookeeper1"
        And we wait "10.0" seconds
        Then zookeeper "zookeeper1" has no value for key "/pgconsul/postgresql/maintenance/master"

    Scenario: No splitbrain in maintenance mode after failover
        Given a "pgconsul" container common config
        """
            pgconsul.conf:
                global:
        ...
        Then zookeeper "zookeeper1" has value "2" for key "/pgconsul/postgresql/timeline"
        When we set value "enable" for key "/pgconsul/postgresql/maintenance" in zookeeper "zookeeper1"
