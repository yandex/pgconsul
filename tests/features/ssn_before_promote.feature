Feature: SSN is set before promote to prevent data-loss window
    ...
    # ---------------------------------------------------------------------------
    # Scenario 3: Switchover — SSN is set before the candidate is promoted
    #
    ...
    Scenario: SSN is set before promote during switchover (quorum mode)
        ...
        # TODO: fix test after fixing priorities.
        Then we remember which of "postgresql2,postgresql3" became primary as "new_primary" and the other as "new_replica"
        #Then container "postgresql2" became a primary
        Then container "new_primary" pgconsul log contains messages in order within "60" seconds
        """
        ACTION. Setting SSN before promote
        ACTION. Setting synchronous_standby_names to ANY 1(pgconsul_new_replica_1_pgconsul_pgconsul_net,pgconsul_postgresql1_1_pgconsul_pgconsul_net)
        Set SSN before promote
        ACTION. Starting promote
        """
