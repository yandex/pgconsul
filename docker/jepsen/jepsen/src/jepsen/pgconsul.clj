...
(defn conn-spec
  ...
  {:classname   "org.postgresql.Driver"
   ...
   :subname     (str "//" (name node) ":6432/postgres?prepareThreshold=0")
   :user        "repl"
   :password    "repl"})
  ...
  (reify client/Client
    ...
    (open! [_ test node]
      (let [conn (atom (conn-spec node))]
        (cond (string/includes? (name node) "postgresql")
              (pg-client conn)
              true
              (noop-client))))
    (invoke! [this test op]
               ...
               :kill (assoc op :value
                            (try
                              (let [procname (rand-nth [:postgres
                                                        :pgconsul])
                                    node (rand-nth (filter (fn [x] (string/includes? (name x) "postgresql"))
                                                           (:nodes test)))]
                                (control/on node
    ...
    (invoke! [this test op]
             (case (:f op)
               :switch (assoc op :value
                          (try
                              (let [node (rand-nth (filter (fn [x] (string/includes? (name x) "postgresql"))
                                                           (:nodes test)))]
                                (control/on node
                                  (control/exec :timeout :10 :pgconsul-util :switchover :-y))
...
(defn pgconsul-test
  [pg-nodes zk-nodes]
  {:nodes     (concat pg-nodes zk-nodes)
   :name      "pgconsul"
   :os        os/noop
   :db        (db)
